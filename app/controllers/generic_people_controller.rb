class GenericPeopleController < ApplicationController
  ...
  def search
    ...
    if params[:identifier]
      dummy_identifier_type = PatientIdentifierType.find_by_name("DUMMY ID").id
      dummy_record = PatientIdentifier.find(:last,
        ...
        #******************************************
          ...
          unless birthday_params.empty?
            if birthday_params["birth_year"] == "Unknown"
              PatientService.set_birthdate_by_age(person, birthday_params["age_estimate"], person.session_datetime || Date.today)
            else
          ...
          person.save
          person.names.last.update_attributes(names_params)
          #person.names.create(names_params)
          person.addresses.last.update_attributes(address_params) unless address_params.empty?
