class PrescriptionsController < GenericPrescriptionsController
  ...
  def create_advanced_prescription
    ...
    if(patient)
      print_and_redirect("/patients/dashboard_print_opd_visit/#{patient.id}","/patients/treatment_dashboard/#{patient.id}") and return
    else
      print_and_redirect("/patients/dashboard_print_opd_visit/#{params[:patient_id]}","/patients/treatment_dashboard/#{params[:patient_id]}") and return
    end
