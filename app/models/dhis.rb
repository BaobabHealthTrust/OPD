class Dhis
	...
	def opd_patient_summary(start_date=@start_date, end_date=@end_date, program="OPD PROGRAM")
		...
		PatientProgram.find_by_sql("
			SELECT 
				...
				GROUP_CONCAT(sscn.name) as sec_diagnosis,
				GROUP_CONCAT(dsscn.name) as detailed_sec_diagnosis,
				GROUP_CONCAT(ssscn.name) as specific_sec_diagnosis
			FROM
				(SELECT 
					patient_id,
						encounter_id,
			...
			WHERE
				o.concept_id IN (6542 , 8345, 8348, 6543, 8346, 8347)
