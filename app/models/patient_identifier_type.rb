    ...
    case self.name
      when "National id"
        unless use_moh_national_id
          ...
          last_national_id = PatientIdentifier.find(:last,:order=>"identifier desc", :conditions => ["identifier_type = ? AND left(identifier,5)= ?", self.patient_identifier_type_id, national_id_prefix])
